$(function () {
  ...
  function updateSeatDisplay() {
    // 탑승 인원 업데이트
    let adultPrice = (
      priceType.adult * parseInt($("#count-adult").text())
    ).toLocaleString("ko-KR");
    let childPrice = (
      priceType.child * parseInt($("#count-child").text())
    ).toLocaleString("ko-KR");
    let seniorPrice = (
      priceType.senior * parseInt($("#count-senior").text())
    ).toLocaleString("ko-KR");
    let disabledPrice = (
      priceType.adult * parseInt($("#count-disabled").text())
    ).toLocaleString("ko-KR");
  ...
  $.getJSON("../json/reserve_layout_curr.json", function (data) {
    ...
    currSeat = currSeat.coaches.find((coach) =>
      coach.coachNumber.includes(currentCoachNumber)
    );
  });
  ...
  function resetControls() {
    ...
    const selectedSeats = $("#selected-seats")
      .text()
      .split(",")
      .map((s) => s.trim());
    selectedSeats.forEach((seat) => {
      $(`#${seat}`)
        .removeClass("seat-blue")
        .addClass("seat")
        .css("color", "black");
    });
    $("#selected-seats").text("");
  ...
  function displaySeats(coachNumber) {
    ...
    let coach = trainData.coaches.find((coach) =>
      coach.coachNumber.includes(coachNumber)
    );
    ...
    Object.keys(coach.seats).forEach((row) => {
      coach.seats[row].forEach((seat) => {
        ...
        if (seat.direction === "forward") {
          seatDiv = $(
            `<div class="p-2 ${
              seat.reserved ? "seat-off" : "seat"
            }" id="${seatId}</div>`
          );
        } else {
          seatDiv = $(
            `<div class="p-2 ${
              seat.reserved ? "seat-off-reverse" : "seat-reverse"
            }" id="${seatId}</div>`
          );
        }
        ...
        // 이미 선택된 좌석인 경우
        const alreadySeatId = `${coachNumber}-${row}${
          seat.number < 10 ? "0" : ""
        }${seat.number}`;

        if (selectedSeats.includes(alreadySeatId)) {
          if (seat.direction === "forward") {
        ...
        if (row === "B" && seat.number === maxSeats) {
          container.append(
            '<div class="seat-center d-flex justify-content-evenly align-items-center">' +
              '<span class="loc-seat">' +
              $("#startLoc").text() +
              '</span><span style="color: #3769e5; font-weight: bold;"> ' +
              "> " +
              nbspString +
              "> " +
              nbspString +
  ...
  $("#train-seats").on(
    "click",
    ".seat, .seat-reverse, .seat-blue, .seat-blue-reverse",
    function () {
      if (
        !$(this).hasClass("seat-off") ||
        !$(this).hasClass("seat-off-reverse")
      ) {
        let total = parseInt(sessionStorage.getItem("total") || 0);
        if (
          $(this).hasClass("seat-blue") ||
          $(this).hasClass("seat-blue-reverse")
        ) {
          // 좌석이 이미 선택된 상태이면 해제
          ...
          const seatNumber = clickSeat
            .split("-")[1]
            .split("")
            .slice(1)
            .join("");
          if (
            currSeat.seats[clickSeat.split("-")[1].split("")[0]][seatNumber - 1]
              .reserved === true
          ) {
            alert("이미 예약된 좌석입니다.");
            if (seatNumber <= 8) {
          ...
          if (calculateTotalSeats() > total) {
            ...
            const exceedModal = new bootstrap.Modal(
              document.getElementById("exceedLimitModal")
            );
            exceedModal.show();
          }
        }
  ...
  function calculateTotalSeats() {
    ...
    ["label-adult", "label-child", "label-senior", "label-disabled"].forEach(
      (item) => {
  ...
  $("#btn-reserve").on("click", function () {
    ...
    const totalPassengers =
      passengerData.adult +
      passengerData.child +
      passengerData.senior +
      passengerData.disabled;
    ...
    if (selectedSeats !== totalPassengers) {
      ...
      $("#exceedLimitModal .modal-body").text(
        "선택된 좌석 수와 탑승 인원 수가 일치하지 않습니다."
      );
    } else {
    ...
    $(newWindow).on("load", function () {
      $.ajax({
        ...
        success: function (data) {
          ...
          newWindow.document.getElementById("train-schedule-body").innerHTML =
            tbodyHtml;
        },
